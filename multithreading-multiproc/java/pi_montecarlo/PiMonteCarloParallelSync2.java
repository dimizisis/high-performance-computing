...
class PiMonteCarloParallelSync2 {
    public static void main(String[] args) throws InterruptedException {
      if (args.length != 1) {
          System.out.println("Usage : PiMonteCarloParallelSync2 <num points>");
        System.exit(1);
      }
    ...
    private static class CalcThread extends Thread{
        ...
        public void run(){
            ...
            y = Math.random();
            z = x*x+y*y;
            criticalSection(z, count);
        }

        private static synchronized void criticalSection(double z, int[] count){
            if(z<=1){
                ++count[0];
            }
        }
