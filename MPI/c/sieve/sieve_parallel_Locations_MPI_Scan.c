...
int main(int argc, char *argv[]){
    ...
    else if (rank == 0) ++start;    /* start from 2 */

    /* Each thread will have a locations vector */
    int* locations = array;

    /* Perform sieve */
    sieve(array, N+1, locations, start, stop);
    
    /* MPI_Scan performs MPI_Reduce with priority according to process rank */
    MPI_Scan(locations, array, N+1, MPI_INT, MPI_BAND, MPI_COMM_WORLD);
